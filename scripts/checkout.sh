...
then
	...
	pushd $ROOTDIR/$MAIN/$RDIR
	   ...
	   echo ${row} | base64 --decode | jq --arg comhash "${comhash}" --arg toolchainhash "${toolchainhash}" '. + {documentcommit : $comhash, toolchaincommit: $toolchainhash, hostname: "https://data.vlaanderen.be" }' > .publication-point.json
	   cleanup_directory 
        popd
